...
public class BookflixApplication {
	...
	@Bean
	public CommandLineRunner initTrechos(TrechosRepository repository) {
		return (args) -> {
	...
	@Bean
	public CommandLineRunner initAutores(AutoresRepository repository) {
		return (args) -> {
			log.info("Antes de criar os registros de autores");
			repository.save(new Autor("Machado de Assis"));
			repository.save(new Autor("Lima Barreto"));
			log.info("Depois de criar os registros de autores");
		};
	}
	
	@Bean
	public CommandLineRunner initObras(ObrasRepository repository) {
		return (args) -> {
			log.info("Antes de criar os registros de obras");
			repository.save(new Obra("Triste Fim de Policarpo Quaresma"));
			repository.save(new Obra("Memórias Póstumas de Brás Cubas"));
